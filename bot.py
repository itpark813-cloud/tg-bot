...
# Укажите ваш новый токен из @BotFather
...
FRONTEND_COURSE = {
    ...
    "3": {
        "title": "Урок 3: Flexbox — Выравнивание",
        "text": (
            "<b>УРОК 3: FLEXBOX</b>\n\n"
    ...
    "9": {
        ...
        "text": (
            ...
            "<b>Быстрый старт проекта на Vite:</b>\n"
            "1. Откройте терминал\n"
...
def get_main_keyboard():
    ...
    btn_focus = types.KeyboardButton("ФОКУС-ТАЙМЕР")
    btn_notes = types.KeyboardButton("БЛОКНОТ")
    btn_style = types.KeyboardButton("СТИЛИЗАТОР")
...
def run_pomodoro_timer(chat_id):
    # Фоновый поток для таймера, чтобы бот не зависал
    time.sleep(10)  # Для теста 10 сек (для 25 минут укажите 25 * 60)
...
def send_welcome(message):
    welcome_text = (
        ...
        "<b>Что внутри:</b>\n"
        "- 10 полноценных практических уроков\n"
        "- Инструменты продуктивности и фокуса\n"
        "- Блокнот кода и стилизатор\n\n"
        "Нажми кнопку в меню ниже, чтобы начать обучение."
    )
    bot.send_message(
        message.chat.id, 
        welcome_text, 
...
def handle_menu(message):
    ...
    if user_state.get(chat_id) == 'waiting_style':
        ...
        formatted = (
            f"<b>Исходный текст:</b> <code>{text}</code>\n\n"
            f"<b>Стилизованный формат:</b>\n<code>{spaced_text}</code>\n\n"
            f"<b>В виде кода:</b>\n<code>{text}</code>"
        )
        user_state[chat_id] = None
    ...
    if text == "КУРС FRONTEND (10 УРОКОВ)":
    ...
    elif text == "ФОКУС-ТАЙМЕР":
        markup = types.InlineKeyboardMarkup()
        btn_start = types.InlineKeyboardButton("Запустить 25 мин Помодоро", callback_data="start_pomodoro")
        markup.add(btn_start)
        msg = (
            "<b>ФОКУС-ТАЙМЕР (25 МИНУТ)</b>\n\n"
        ...
        if not notes:
            ...
            notes_text = "\n".join([f"<code>{i+1}.</code> {n}" for i, n in enumerate(notes)])

        msg = f"<b>ВАШИ ЗАМЕТКИ КОДА:</b>\n\n{notes_text}"
        bot.send_message(chat_id, msg, parse_mode="HTML", reply_markup=markup)

    elif text == "СТИЛИЗАТОР":
        user_state[chat_id] = 'waiting_style'
        bot.send_message(
            chat_id, 
            "<b>Пришли любой текст</b>, и я преобразую его в моноширинный код-формат!", 
            parse_mode="HTML"
        )
...
def callback_inline(call):
    ...
    if data.startswith("lesson_"):
        ...
        if lesson:
            markup = types.InlineKeyboardMarkup()
            btn_back = types.InlineKeyboardButton("Назад к списку уроков", callback_data="back_to_course")
            markup.add(btn_back)
        ...
        threading.Thread(target=run_pomodoro_timer, args=(chat_id,)).start()
